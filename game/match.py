    ...
    if world == 1:
        ...
        while True:
            ...
            if not skip_u:
                ...
                # 1e. Show enemy health, name and stats
                name = enemy.name
                match enemy.diff:
                ...
                print("Enemy +DMG: " + str(damage_inc_e) + " | Enemy Poison: " + str(poison_e) + " | + Crit %: " + str(crit_inc_e))
                ...
                match target_cat:
                    case "ATK":
                        ...
                        if defense_e: print(f"{name}{styles.clear_styles()} {styles.format_style("defended", "cyan")}{styles.clear_styles()}! (card base dmg / {def_div:.1f})"); defense_e -= 1
                    ...
                    case "WPN":
                        ...
                        if target["type"] in enemy.weak_el + enemy.weak_mat:
                            ...
                            print(f"{name}{styles.clear_styles()} took {additive} damage due to being weak to {target["type"]}!")
                            time.sleep(3)
                        elif target["type"] in enemy.res_el + enemy.res_mat:
                            additive = 5 + random.randint(-1, 1)
                            op_health += additive
                            print(f"{name}{styles.clear_styles()} gained {styles.format_style(f"{additive} health", "green")}{styles.clear_styles()} due to being resistant of {target["type"]}!")
                            time.sleep(3)
                    ...
                    case "AMR":
                        ...
                        for effect_type, value in effects.items():
                            match effect_type:
                                ...
                                case "defend_round":
                                    defense_u += value
                                    print(f"You're now {styles.format_style("defending", "cyan")}{styles.clear_styles()} against {name}{styles.clear_styles()} for {value} rounds!")
                                    time.sleep(3)
                                case "crit_inc":
                                    crit_inc_u += value
                                    print(f"{styles.format_style("Crit %", "yellow")}{styles.clear_styles()} increased by {styles.format_style(str(value), "green")}{styles.clear_styles()}!")
                                    time.sleep(3)
                ...
                helper.clear()
                print(f"Your turn was skipped by {name}{styles.clear_styles()}!")
                skip_u -= 1
                time.sleep(2)

            if skip_e: name = helper.get_name_colored(enemy); skip_e -= 1; print(f"{name}{styles.clear_styles()}'s turn was skipped!"); time.sleep(2); continue
            ...
            match enemy_card_category:
                case "ATK":
                    ...
                    if defense_u: print(f"You {styles.format_style("defended", "cyan")}{styles.clear_styles()}! (DMG / {def_div:.1f})"); defense_u -= 1
                    if crit: print(f"{name}{styles.clear_styles()} hit you with a {styles.format_style("critical hit!", "red")}{styles.clear_styles()} (x{crit_perc})")
                    time.sleep(3)
                case "WPN":
                    # 3. APPLY EFFECTS
                    effect = target["effect"]
                    ...
                    for effect_type, value in effects.items():
                        match effect_type:
                            ...
                            case "damage_increase": 
                                damage_inc_e += value
                                print(f"{name}{styles.clear_styles()} {styles.format_style("damage", "red")}{styles.clear_styles()} was increased! (+{value}, total +{damage_inc_e})")
                                time.sleep(3)
                            # 3b. Poisoning
                            case "poison": 
                                poison_u += value
                                print(f"{name}{styles.clear_styles()} {styles.format_style("poisoned", "progress")}{styles.clear_styles()} you! (+{value}, total {poison_u})")
                                time.sleep(3)
                            # 3c. Player turn skip
        ...
        for c_round in range(3):
            ...
            while True:
                ...
                if not skip_u:
                    print(styles.format_style(f"Round: {c_round + 1}", "bold_cyan"))
                    # 1b. Show player health & effects
                    print("Health: " + health_colored)
                    print("+DMG: " + str(damage_inc_u) + " | Poison: " + str(poison_u) + " | + Crit %: " + str(crit_inc_u))
                    # 1c. Show player colored deck
                    ...
                    if op_health <= 0: wins[c_round] = 1; break
                ...
                if skip_e: 
                    ...
                    helper.clear()
                    skip_e -= 1
                    print(f"{name}{styles.clear_styles()}'s turn was skipped!")
                    time.sleep(2)
                    continue
                ...
                match enemy_card_category:
                    case "ATK":
                        ...
                        print(f"{name}{styles.clear_styles()} played {helper.format_card_line(enemy_card, cards)}{styles.clear_styles()}!")
                        print(f"You took {styles.format_style(str(damage), "red")} damage! {"(" if modifs != "" else ""}{modifs}{")" if modifs != "" else ""}")
                        if defense_u: print(f"You {styles.format_style("defended", "cyan")}{styles.clear_styles()}! (card base dmg / {def_div:.1f})"); defense_u -= 1
                        if crit: print(f"{name}{styles.clear_styles()} hit you with a {styles.format_style("critical hit!", "red")}{styles.clear_styles()} (x{crit_perc})")
                        time.sleep(3)
                    case "WPN":
                        # 3. APPLY EFFECTS
                ...
                if u_health <= 0: wins[c_round] = 0; break

            if wins[c_round] == 1:
                helper.clear()
                print(styles.format_style("You won the round!", "success"))
                print(styles.format_style("Press any key to continue...", "warn"))
                readchar.readkey()
            else:
                helper.clear()
                print(styles.format_style("You lost the round...", "red"))
                print(styles.format_style("Press any key to continue...", "warn"))
                readchar.readkey()
